...
/// <summary>
/// InMemory implementation — chạy được ngay không cần PostgreSQL.
/// Production thay bằng EF Core + PostgreSQL.
/// </summary>
public class InMemoryConfigRepository : IConfigRepository
{
    ...
    public Task<List<ConnectorConfig>> GetAllAsync(string? status, string? connectorType)
    {
        ...
        // Chỉ trả version mới nhất per id
        var configs = query
            .GroupBy(c => c.Id)
...
public class InMemoryConfigAuditRepository : IConfigAuditRepository
{
    ...
    public Task<List<ConfigAuditEntry>> GetAudiConfigAsync(string configId)
    {
        var entries = _entries.Where(e => e.ConfigId == configId)
                                    .OrderByDescending(e => e.Timestamp)
                                    .ToList();
        return Task.FromResult(entries);
    }
